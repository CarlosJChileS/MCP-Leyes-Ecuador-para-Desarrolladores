...
export function assessProject(project: ProjectProfile, sources: LegalSource[], language: Language = 'es') {
  ...
  return { project: project.name, language, risks, controls, questions, references: relevant.map(({ id, title, url, verifiedAt, status, summary }) => ({ id, title, url, verifiedAt, status, summary })), obligations, complianceScores, improvementPlan, disclaimer: localized(language, 'Orientación preliminar; no constituye dictamen ni certificación jurídica.', 'Preliminary guidance; this is not a legal opinion or certification.') };
}
export function auditChecklist(project: ProjectProfile, sources: LegalSource[], language: Language = 'es') { const a = assessProject(project, sources, language); return { project: a.project, language, items: [...a.obligations.map((obligation) => ({ id: obligation.id, text: obligation.requirement, norm: obligation.normTitle, article: obligation.article, whyItApplies: obligation.whyItApplies, evidence: obligation.evidence, gap: obligation.gap, recommendedActions: obligation.recommendedActions, status: obligation.status, legalReviewRequired: obligation.legalReviewRequired, priority: obligation.priority, risk: obligation.risk, owner: obligation.owner, dueDate: obligation.dueDate, closureCriterion: obligation.closureCriterion, sourceUrl: obligation.sourceUrl })), ...a.controls.map((text) => ({ text, evidence: localized(language, 'Definir evidencia y responsable', 'Define evidence and owner'), status: 'pendiente' as const, priority: 'media' as const })), { text: localized(language, 'Revisar fuentes y vigencia', 'Review sources and legal status'), evidence: a.references.map((r) => r.url).join(', '), status: 'pendiente' as const, legalReviewRequired: true, priority: 'alta' as const }], scores: a.complianceScores, improvementPlan: a.improvementPlan, disclaimer: a.disclaimer }; }
