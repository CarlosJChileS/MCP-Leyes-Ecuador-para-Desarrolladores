...
export function assessProject(project: ProjectProfile, sources: LegalSource[], language: Language = 'es') {
  ...
  const complianceScores = Object.fromEntries(scoreDimensions.map(dimension => {
    ...
    return [dimension, { score: applicable.length ? Math.round((completed / applicable.length) * 100) : null, status: applicable.length ? 'evaluada' : 'no_evaluada', obligations: applicable.length, completed, pending: applicable.filter(item => item.status !== 'cumple' && item.status !== 'no_aplica').length }];
  }));
